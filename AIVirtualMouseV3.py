...
while True:
    ...
    if len(lmList) != 0 and len(plmList) != 0:
        ...
        xm = plmList[11][1]
        ym = plmList[11][2]
        dist = int(((((plmList[12][1] - xm)**2) + ((plmList[12][2] - ym)**2))**0.5) / 2)

        # 3. check which fingers are up
        fingers = detector.fingersUp()
        # cadre du mousePad
        cv2.rectangle(img, (xm, ym - (dist * 2)), (xm + dist, ym - dist), (255, 0, 255), 2)

        # 4. only index finger : moving mode
        if fingers[1] == 1 and fingers[0] == 1:
            # 5. convert coordinates
            x3 = np.interp(x1, (xm, xm + dist), (0, wScr))
            y3 = np.interp(y1, (ym - (dist * 2), ym - dist), (0, hScr))
        ...
        if fingers[1] == 0 and fingers[0] == 1:
            length, img, lineInfo = detector.findDistance(5, 8, img)
